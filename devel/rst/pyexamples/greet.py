...
def main():
    ...
    instructions = Text(Point(win.width/2, 40),
                     "Enter your name.\nThen click the mouse.")
    instructions.draw(win)

    entry1 = Entry(Point(win.width/2, 200),10)
    entry1.draw(win)

    Text(Point(win.width/2, 230),'Name:').draw(win) # label for the Entry
    ...
    Text(Point(win.width/3, 150), greeting1).draw(win)
                     
    greeting2 = 'Bonjour, ' + name + '!'
    Text(Point(2*win.width/3, 100), greeting2).draw(win)
